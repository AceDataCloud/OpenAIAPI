...
The `data` contains specific information about the images generated by the model, and the `url` inside it is the detailed link to the generated image, as shown in the figure.
...
The size of the image is set to `1024 * 1024`, with specific settings as shown in the figure below:
...
The image style parameter is set to `vivid`, with specific settings as shown in the figure below:
...
The image link format parameter is set to `url`, with specific settings as shown in the figure below:
...
With the same operation as above, simply changing the image link format parameter to `b64_json` will yield the Base64 encoded image link, with the specific result shown below:
