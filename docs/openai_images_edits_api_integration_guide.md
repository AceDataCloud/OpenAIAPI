...
To use Python for the call, we first need to import two environment variables: one `OPENAI_BASE_URL`, which can be set to `https://api.acedata.cloud/openai`, and another for the credential variable `OPENAI_API_KEY`, which is the value obtained from `authorization`. On Mac OS, you can set the environment variables with the following commands:
...
After the call, we will find that an image `gift-basket.png` is generated in the current directory, with the specific result shown below:
